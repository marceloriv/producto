...
public class ProductoExceptionHandler {
    ...
    public ResponseEntity<ApiRespuestaDto> handleValidationExceptions(MethodArgumentNotValidException ex) {
        String mensaje = "Error de validación en el módulo de producto";
     ...
     * Esta excepción se lanza cuando ocurre un error inesperado en el módulo de producto.
     */
    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiRespuestaDto> handleGenericException(Exception ex) {
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ApiRespuestaDto(ApiRespuestaEstados.ERROR, "Error interno en el módulo de producto"));
    }
}
